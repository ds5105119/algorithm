...
def insert(node, x, y):
    ...
    if direction == 1:
        if not node['NE']:
            node['NE'] = quad_node(x, y, node['x'], node['r'], node['y'], node['t'])
        else:
            node['NE'] = insert(node['NE'], x, y)
    elif direction == 2:
        if not node['NW']:
            node['NW'] = quad_node(x, y, node['l'], node['x'], node['y'], node['t'])
        else:
            node['NW'] = insert(node['NW'], x, y)
    elif direction == 3:
        if not node['SW']:
            node['SW'] = quad_node(x, y, node['l'], node['x'], node['b'], node['y'])
        else:
            node['SW'] = insert(node['SW'], x, y)
    elif direction == 4:
        if not node['SE']:
            node['SE'] = quad_node(x, y, node['x'], node['r'], node['b'], node['y'])
        else:
...
def main():
    ...
    for i in range(test_count):
        ...
        root = quad_node(*sampled_points[0], 0, 1e10, 0, 1e10)
